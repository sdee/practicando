#!/bin/bash
echo "🧪 Starting TEST server (test DB) + Frontend"
...
# Load test environment if it exists, otherwise set sensible defaults
if [ -f "$ENV_FILE" ]; then
    set -a
    source "$ENV_FILE"
    set +a
    echo "📋 Loaded .env.test configuration from: $ENV_FILE"
else
    export DATABASE_MODE=test
    export ENVIRONMENT=test
    export DEBUG=true
    echo "⚠️  .env.test not found at: $ENV_FILE, using fallback settings"
fi
